...
type ClaudeExecutor interface {
	// ExecuteClaude executes Claude with the given prompt and optional session ID.
	// Returns a channel that streams ClaudeResponse events.
	// The channel will be closed when the execution completes.
	ExecuteClaude(ctx context.Context, prompt string, sessionID string) (<-chan ClaudeResponse, error)
