...
type ClaudeExecutor interface {
    ExecuteClaude(ctx, prompt, sessionID, isNewSession, model, customInstructions) (<-chan ClaudeResponse, error)
...
**Client -> Server:**
```json
{"type": "message", "content": "...", "session_id": "uuid", "attachments": []}
```
Note: `session_id` is always present (generated by frontend at startup).
