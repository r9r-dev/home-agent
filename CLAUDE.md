...
File Upload Feature (v0.11.0, updated v0.13.2):
...
Bug Fixes (v0.13.2):
- Fixed: File uploads now always have a valid session_id (eliminates `temp/` folder)
- Fixed: Response paragraph separation when Claude responds in multiple parts
- Changed: Session ID now generated by frontend and passed to Claude via `--session-id`
- Changed: Unified internal and Claude CLI session IDs (simplified architecture)
...
- Key files:
  ...
  - `claude-proxy/services/claude.go` - Parses `thinking_delta` events from CLI
...
### Session Management (v0.14.0+)
- Frontend generates `session_id` (UUID) at startup and on new conversation
- Session ID passed to Claude CLI via `--session-id <uuid>` for new sessions
- Session resumed via `--resume <uuid>` for existing sessions
- No separate `claude_session_id` needed - internal and CLI session IDs are now unified
- File uploads always have a valid session_id (no more `temp/` folder)
...
**Client -> Server:**
```json
{"type": "message", "content": "...", "sessionId": "uuid", "model": "haiku", "attachments": [], "thinking": false}
```
Note: `sessionId` is always present (generated by frontend at startup). `thinking` enables extended thinking mode.
